...
import { NocapPostLike } from '@/core/models/section-c';
...
interface PostLikesBottomSheetProps {
...
const PostLikesBottomSheet = React.forwardRef<RBSheetRef, PostLikesBottomSheetProps>(
  ({ nocapPostLikes, currentUserId, localLiked }, ref) => {
    ...
    const fetchLikers = async () => {
      setLoading(true);
      const liked = nocapPostLikes
        .filter((l) => l.isLiked && l.userId)
      ...
      if (localLiked && !currentUserInList) {
        liked.push({ userId: currentUserId, isLiked: true, id: 'local' });
      }
    ...
    // When more emotions are added, push entries here and filter filteredLikers by emotionId
