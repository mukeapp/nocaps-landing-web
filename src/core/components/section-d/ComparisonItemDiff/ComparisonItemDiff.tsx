...
import { ComparisonHabitLinkItem } from '@/core/models/section-b/habit';

export interface ComparisonItemDiffProps {
  cmp: ComparisonHabitLinkItem;
  costSymbol?: string;
}

const ComparisonItemDiff: React.FC<ComparisonItemDiffProps> = ({ cmp, costSymbol = "" }) => {
  const { save, currentItem, swapItem } = cmp;
  const swapCost = swapItem?.cost ?? 0;
...
export default ComparisonItemDiff;
