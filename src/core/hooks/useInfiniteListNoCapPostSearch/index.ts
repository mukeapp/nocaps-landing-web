...
export function useInfiniteListNoCapPostSearch<T>(fetchPage: PageFetcherNocapPost<T>, postVisibility: number) {
  ...
  const loadNext = useCallback(async () => {
    ...
    try {
      ...
      if (gen !== generationRef.current) return;
      console.error("useInfiniteListNoCapPostSearch loadNext error:", err);
    } finally {
