...
import { useFriendRequestForm } from "@/core/hooks";
import { FriendsVerticalList } from "@/core/components/section-b-1";

const FriendsRequestScreen: React.FC<{ navigation: any; route: any }> = ({
  navigation,
  route,
}) => {
  const form = useFriendRequestForm({ navigation, route });
  ...
  return (
    <View style={MainStyles.root2}>
      ...
      <FriendsVerticalList
        ...
        showEmail={false}
        onAddFriend={(f) => {
          if (f?.friend?.isReceiver) {
            form.acceptFriendRequest(f?.userId, f?.friend?.documentId);
          }
        }}
        onRemoveFriend={(f) => {
