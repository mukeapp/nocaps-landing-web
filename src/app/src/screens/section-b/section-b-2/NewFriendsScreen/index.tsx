...
import { useNewFriendsForm } from "@/core/hooks";
import { FriendsVerticalList } from "@/core/components/section-b-1";

const NewFriendsScreen: React.FC<{ navigation: any; route: any }> = ({
  navigation,
  route,
}) => {
  const form = useNewFriendsForm({ navigation, route });
  ...
  return (
    <View style={MainStyles.root2}>
      ...
      <FriendsVerticalList
        ...
        showEmail={false}
        onAddFriend={(f) => {
          form.sendFriendRequest(f?.userId);
        }}
        onRemoveFriend={(f) => {
          form.removeFriend(f?.userId);
        }}
        onOpenProfile={(f) => {
          navigation.navigate("profile", {
            originScreen: "new-friends",
